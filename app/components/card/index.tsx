...
function StepCard({ title, desc, icon }: StepCardProps) {
  ...
  background: "linear-gradient(90deg, #0C1B2A 80%, #A2B8CE 280%)",
    ...
    >
      ...
      className="text-[36px]  leading-[32px] font-[600] text-white">{title}</h3>
      <p 
      style={{fontFamily:'DM Sans'}}
      className="font-[400] text-[28px] leading-[38px] text-[#CECECE]">{desc}</p>
    </div>
  );
}
...
export default function WeStaySection() {
  return (
    <section className="w-full bg-[#0C1B2A] py-[136px]">
      <div className="mx-auto flex flex-col gap-[50px] max-w-7xl ">
        <h2
          style={{ fontFamily: "DM Sans" }}
          className="text-center text-[52px] leading-[44px]  font-[500] text-white"
        >
